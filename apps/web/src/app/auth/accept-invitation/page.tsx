...
function AcceptInvitationContent() {
  ...
  useEffect(() => {
    ...
    const validateToken = async () => {
      try {
        const response = await fetch(`/api/auth/validate-token?token=${token}&token_type=invitation`);
        const data = await response.json();

        if (!data.valid) {
          setValidationError(data.error_message || 'Invalid or expired invitation');
        } else {
          setInvitationData({
            email: data.email,
      ...
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardTitle className="text-2xl font-bold">
            Welcome{invitationData?.first_name ? `, ${invitationData.first_name}` : ''}!
          </CardTitle>
          <CardDescription>
            Set your password to access your SpotFunnel dashboard
          </CardDescription>
          {invitationData?.email && (
            <p className="text-sm text-gray-600 mt-2">{invitationData.email}</p>
          )}
        </CardHeader>
