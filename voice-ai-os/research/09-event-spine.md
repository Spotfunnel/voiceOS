...
**🟢 LOCKED** - Production-validated research based on Hamming.ai five-layer observability (1M+ calls), OpenTelemetry, event sourcing, distributed tracing, 4x faster incident resolution with events vs logs. Updated February 2026.

---

## Why This Matters for V1

Event spine architecture is the observability and debugging foundation that separates functional voice AI prototypes from production-grade systems capable of operating at scale. The core problem: **logs alone cannot debug distributed, real-time voice AI systems—they lack correlation, ordering guarantees, and replay capability**. Production data from 2025-2026 reveals that teams relying only on logs face three critical failures: (1) **inability to debug**—mean time to debug (MTTD) is 2-4 hours because logs from different components (STT, LLM, TTS) cannot be correlated, (2) **inability to analyze**—product teams cannot answer questions like "why did task completion drop 15%?" because logs lack structured analytics, and (3) **inability to replay**—cannot reproduce production issues locally because logs don't capture causal event sequences.
...
1. **No Correlation**: Logs from STT, LLM, TTS are separate streams—cannot correlate events from single conversation without correlation IDs
2. **No Ordering Guarantees**: Logs may arrive out of order due to buffering, network delays—cannot reconstruct event sequence reliably
3. **No Replay Capability**: Logs describe what happened but don't capture causal event sequences needed to reproduce issues
4. **Unstructured**: Traditional logs lack schemas—difficult to query for analytics ("what percentage of calls had interruptions?")
...
**Production Pattern:**
- Metrics for alerting (P95 latency >800ms → alert)
...
Call Start (generate trace_id)
  → STT (emit event with trace_id)
    → LLM (emit event with trace_id)
      → Tool Gateway (emit event with trace_id)
        → Tool Execution (emit event with trace_id)
      → LLM Response (emit event with trace_id)
    → TTS (emit event with trace_id)
  → Audio Delivery (emit event with trace_id)
```
...
- **Limitation**: Clock skew between servers can cause ordering issues—sequence numbers are more reliable
...
- Enables reconstructing causal chain: user_spoke → transcript_ready → llm_invoked → llm_responded → tts_started → agent_spoke
...
- Winners in 2025-2026 consistently measure, iterate, and optimize—not just deploy
...
- Propagate through all API calls: STT → LLM → Tool Gateway → Tool → LLM → TTS
- Include trace_id in all events, logs, metrics
- **Performance overhead**: Expect 1-5% latency from tracing instrumentation—worthwhile tradeoff for debugging capability
...
- Use ISO8601 format with millisecond precision (or microsecond for high-frequency events)
- Include timezone (UTC recommended)
- **Limitation**: Clock skew between servers can cause ordering issues—sequence numbers more reliable
...
- Cannot rely on deployment-only metrics (uptime, response times)—must measure business outcomes
...
- Logs are unstructured—cannot aggregate for percentages, averages, trends
- Logs lack business context—cannot determine task success from technical logs
- Logs lack correlation—cannot join STT confidence with task success
- Logs lack segmentation—cannot filter by customer, region, agent version
...
- Track causal chain: user_spoke → transcript_ready → llm_invoked → tool_executed → llm_responded → agent_spoke
...
- Build causal chain: user_spoke → transcript_ready → llm_invoked → tool_executed → llm_responded → agent_spoke
